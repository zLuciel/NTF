...
const Card = ({id,user,data,edit}) => {
  ...
  return (
    <NewCard>
      ...
      <ImgCardNew>
        <Image sizes="(min-width: 808px) 50vw, 100vw"
              fill={true} src={ImageUrl} alt="creadores" />
      </ImgCardNew>
      <div className="grid-center">
        <InfoCardNew>
          {
            <Link prefetch={false} href={`/evento/[id]`} as={`/evento/${id}`}>
              <h1>{data.title ? data.title : "Indonesia - korea Conference "} </h1>
            </Link>
          }
          {<Link prefetch={false} href={`/dashboard/[id]`} as={`/dashboard/${user?._id}`}><UserSpan userAvatar={data.image} /></Link>}
        </InfoCardNew>
